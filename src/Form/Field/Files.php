...
EOT;
	...
	protected function prepareEach($file)
	{
		if(array_get($file, Form::REMOVE_FLAG_NAME) == 1){
			$this->getFiesController()->destroy( array_get($file, 'id'));

			return $file;
		}

		return $this->getFiesController()->upload($file);
	}
	...
	protected function getFiesController()
	{
		if(!$this->filesController){
			...
			$this->filesController = new FilesControl($model->getTable(), $model->getKey());
		}
